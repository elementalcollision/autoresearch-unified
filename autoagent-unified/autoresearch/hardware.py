# Ported from autoresearch-unified/tui/hardware.py (MIT)
"""Hardware info for display — supports Gaudi HPU, NVIDIA/AMD GPU, and Apple Silicon."""
...
def _get_nvidia_info() -> dict | None:
    """Try to get NVIDIA GPU info via backends module."""
    try:
...
def _get_apple_silicon_info() -> dict:
    ...
    info = {'chip_name': 'Unknown', 'gpu_cores': 0, 'memory_gb': 0, 'peak_tflops': 0.0}
    ...
    chip = info['chip_name'].lower()
    flops_per_core = {"m1": 0.5e12, "m2": 0.55e12, "m3": 0.65e12, "m4": 0.7e12, "m5": 0.85e12}
    gen_match = re.search(r'(m[1-9])', chip)
...
def get_hardware_summary() -> dict:
    ...
    return {'chip_name': 'Unknown', 'gpu_cores': 0, 'memory_gb': 0, 'peak_tflops': 0.0}
