"""
MuonAdamW optimizer for CUDA backend.
Ported from muon_mps.py with CUDA-specific optimizations:
- @torch.compile decorators for kernel fusion and CUDA graph capture
- Scalar tensors on CUDA device (no per-step CPU→GPU transfer)
- bf16 tensor core utilization in Newton-Schulz orthogonalization
"""
...
# ---------------------------------------------------------------------------
# Step functions with torch.compile for CUDA kernel fusion
# ---------------------------------------------------------------------------

@torch.compile
def adamw_step(p, grad, exp_avg, exp_avg_sq, step_t, lr_t, beta1_t, beta2_t, eps_t, wd_t):
    """AdamW update step, CUDA-optimized with torch.compile."""
    step = step_t
    lr = lr_t
    beta1 = beta1_t
    beta2 = beta2_t
...
def muon_step(stacked_grads, stacked_params, momentum_buffer, second_momentum_buffer,
    ...
    """Muon update step with Newton-Schulz orthogonalization, CUDA-optimized."""
    # Nesterov momentum
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    CUDA-optimized with torch.compile and device-local scalar tensors."""
