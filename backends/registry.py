...
Maps backend names to training script paths, FLOPS lookup tables, display names,
and hyperparameter suggestion functions. This is the single point where new
platforms are registered.

To add a new platform:
1. Add an entry to PLATFORMS below
2. Add detection logic to backends/__init__.py detect_backend()
3. Add a training script under platforms/<name>/
4. Add a muon optimizer variant in backends/muon_<name>.py
"""

import os
from pathlib import Path

# Resolve project root (one level up from backends/)
...
PLATFORMS = {
    ...
    "rocm7": {
        ...
        "flops_table": None,  # shares rocm's table
    },
    "mlx": {
        "training_script": "platforms/metal/train_mlx.py",
        "display_name": "Apple MLX",
        "flops_table": None,  # computed dynamically from gpu_cores
    },
    "mps": {
        "training_script": "platforms/metal/train_mlx.py",  # MPS uses same script, dispatched internally
...
def get_training_script(backend: str) -> str:
    """Get the training script path for a backend (relative to project root)."""
    if backend not in PLATFORMS:
        raise ValueError(
            f"Unknown backend '{backend}'. Available: {', '.join(PLATFORMS.keys())}"
        )
...
def get_platform_flops(backend: str, chip_name: str) -> float | None:
    """Look up peak bf16 FLOPS for a chip on a specific platform.

    Returns None if not found (caller should fall back to estimation).
    """
    chip_lower = chip_name.lower()

    # rocm7 shares rocm's FLOPS table
