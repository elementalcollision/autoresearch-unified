"""
MuonAdamW optimizer for ROCm 7.x backend.
Ported from muon_rocm.py for AMD Instinct GPUs on ROCm 7.x:
- @torch.compile decorators for kernel fusion via AMD Triton backend
- Scalar tensors on device (no per-step CPU→GPU transfer)
- bf16 matrix core utilization in Newton-Schulz orthogonalization
- Set AUTORESEARCH_NO_COMPILE=1 to disable torch.compile for debugging
"""
...
# ---------------------------------------------------------------------------
# Step functions with torch.compile for AMD Triton kernel fusion
# ---------------------------------------------------------------------------

@_maybe_compile
def adamw_step(p, grad, exp_avg, exp_avg_sq, step_t, lr_t, beta1_t, beta2_t, eps_t, wd_t):
    """AdamW update step, ROCm-optimized with torch.compile."""
    step = step_t
    lr = lr_t
    beta1 = beta1_t
    beta2 = beta2_t
...
def muon_step(stacked_grads, stacked_params, momentum_buffer, second_momentum_buffer,
    ...
    """Muon update step with Newton-Schulz orthogonalization, ROCm-optimized."""
    # Nesterov momentum
    momentum_f = momentum_t
    ...
    # Polar express orthogonalization — bf16 for tensor core utilization
    X = g.bfloat16()
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    ROCm 7.x optimized with torch.compile (AMD Triton) and device-local scalar tensors."""

    def __init__(self, param_groups, device="cuda"):
        super().__init__(param_groups, defaults={})
        # 0-D tensors on CUDA device directly (no CPU→GPU transfer per step)
