...
This version supports CUDA, ROCm, HPU, PyTorch MPS, and MLX backends.
"""
...
def download_data(num_shards, download_workers=8):
    ...
    # Gaudi HPU requires spawn multiprocessing context
    import platform
    if os.environ.get("AUTORESEARCH_BACKEND", "").lower() == "hpu" or platform.system() == "Linux":
        import multiprocessing as mp
        ctx = mp.get_context("spawn")
        pool_cls = ctx.Pool
    else:
        pool_cls = Pool
    with pool_cls(processes=workers) as pool:
...
def make_dataloader(tokenizer, B, T, split, buffer_size=1000, backend=None):
    ...
    backend: 'cuda', 'hpu', 'mps', 'mlx', or None (auto-detect)
    """
    if backend is None:
        if _USE_TORCH:
            import torch
...
def evaluate_bpb(model, tokenizer, batch_size, backend=None):
    ...
    if backend is None:
        if _USE_TORCH:
            ...
            if hasattr(_torch, 'hpu') and _torch.hpu.is_available():
                backend = "hpu"
