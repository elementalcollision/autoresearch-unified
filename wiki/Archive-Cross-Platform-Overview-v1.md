# Cross-Platform Overview (v1 Archive)

> **ARCHIVED**: This page contains results from the pre-unification era (4 separate repositories: autoresearch, autoresearch-cuda, autoresearch-rocm, autoresearch-gaudi). These runs used different codebases with known data loss bugs that were fixed in the unified repository. Results here may be unreliable — see the [current Cross-Platform Overview](Cross-Platform-Overview) for validated data from the unified codebase.
