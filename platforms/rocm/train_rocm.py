"""
Autoresearch pretraining script — ROCm backend.
Single-GPU, single-file (plus backends/ for optimizer and hardware detection).
Optimized for AMD Instinct GPUs: torch.compile, SDPA attention, bf16 autocast.
Usage: AUTORESEARCH_BACKEND=rocm uv run train.py
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # SDPA — on ROCm this dispatches to CK-based attention or mem-efficient kernel
...
WINDOW_PATTERN = "LLSS" # sliding window pattern: L=full, S=half context

# Optimization
TOTAL_BATCH_SIZE = _hp_defaults['total_batch_size']
EMBEDDING_LR = 0.35     # learning rate for token embeddings (Adam)
UNEMBEDDING_LR = 0.008  # learning rate for lm_head (Adam)
MATRIX_LR = 0.044       # learning rate for matrix parameters (Muon)
SCALAR_LR = 0.35        # learning rate for per-layer scalars (Adam)
WEIGHT_DECAY = 0.1      # cautious weight decay for Muon
ADAM_BETAS = (0.65, 0.90) # Adam beta1, beta2
...
# ROCm device setup — HIP exposes the same torch.cuda.* API
device = torch.device("cuda")
device_type = "cuda"

# bf16 autocast — native on MI300x matrix cores
...
print(f"Backend: ROCm ({_hw_info['chip_name']})")
...
# Build model on ROCm (HIP device)
