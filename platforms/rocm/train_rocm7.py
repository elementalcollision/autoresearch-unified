"""
Autoresearch pretraining script — ROCm 7.x backend.
Single-GPU, single-file (plus backends/ for optimizer and hardware detection).
Optimized for AMD Instinct GPUs on ROCm 7.x: torch.compile (reduce-overhead), CK Flash Attention, bf16 autocast.
Usage: AUTORESEARCH_BACKEND=rocm7 uv run train.py
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # SDPA — on ROCm 7.x this dispatches to CK-based attention (explicitly selected)
...
# ROCm 7.x device setup — HIP exposes the same torch.cuda.* API
device = torch.device("cuda")
device_type = "cuda"

# bf16 autocast — native on MI300x/MI325x matrix cores
autocast_ctx = torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16)

# ROCm 7.x: explicitly select CK as the preferred Flash Attention backend
try:
    torch.backends.cuda.preferred_rocm_fa_library("ck")
    print("CK Flash Attention backend: selected")
except AttributeError:
    print("CK Flash Attention backend: API not available (requires PyTorch built for ROCm 7.x)")

# Log SDPA attention backend status
...
print(f"Memory-efficient SDPA:  {'enabled' if mem_efficient_sdp else 'disabled'}")

PEAK_FLOPS = get_peak_flops(_hw_info)
rocm_ver = get_rocm_version()
rocm_str = f"ROCm {rocm_ver[0]}.{rocm_ver[1]}" if rocm_ver else "ROCm 7.x"
print(f"Backend: {rocm_str} ({_hw_info['chip_name']})")
print(f"HBM: {_hw_info['memory_gb']:.0f} GB")
...
# Build model on ROCm 7.x (HIP device)
...
# torch.compile for ROCm 7.x — tiered fallback chain
# ROCm 7.x has mature HIP graph support, enabling CUDA-graph-equivalent optimizations
_COMPILE_BACKEND = os.environ.get("AUTORESEARCH_COMPILE_BACKEND", "inductor")
_COMPILE_MODE = os.environ.get("AUTORESEARCH_COMPILE_MODE", "reduce-overhead")

# Inductor env var workarounds for CDNA shape inference bugs
...
if os.environ.get("AUTORESEARCH_NO_COMPILE", "") == "1":
    ...
    _compile_ok = False
    # Tier 1: Try requested backend + mode (default: inductor/reduce-overhead)
    try:
