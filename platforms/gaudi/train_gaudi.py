"""
Autoresearch pretraining script — Intel Gaudi 3 HPU backend.
Single-HPU, single-file (plus backends/ for optimizer and hardware detection).
Usage: python train_gaudi.py
"""
...
# Initialize Gaudi HPU backend before any torch operations
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # Use PyTorch SDPA — Gaudi 3 HPU backend optimizes this
        # Try FusedSDPA for full-context, fall back to SDPA with mask for sliding window
        ...
        if window > 0 and window < T:
            ...
            # Full causal attention — try FusedSDPA if available
            try:
...
# Gaudi 3 HPU device setup
device = torch.device("hpu")
device_type = "hpu"

# HPU autocast: bfloat16 (native on Gaudi 3 matrix engines)
autocast_ctx = torch.amp.autocast(device_type="hpu", dtype=torch.bfloat16)

PEAK_FLOPS = get_peak_flops(_hw_info)
print(f"Backend: HPU ({_hw_info['chip_name']})")
print(f"Peak bf16 FLOPS: {PEAK_FLOPS:.2e}")
print(f"Devices: {_hw_info.get('device_count', 1)}")
...
# Compile model with Gaudi HPU backend for graph capture and kernel fusion
model = torch.compile(model, backend="hpu_backend")

train_loader = make_dataloader(tokenizer, DEVICE_BATCH_SIZE, MAX_SEQ_LEN, "train", backend="hpu")
...
model.eval()
with autocast_ctx:
    val_bpb = evaluate_bpb(model, tokenizer, DEVICE_BATCH_SIZE, backend="hpu")
